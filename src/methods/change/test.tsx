...
describe(".change()", () => {
  ...
  it("works with uncontrolled checkboxes", async () => {
    ...
    expect((input.get(0) as HTMLInputElement).checked).toBe(true);
  ...
  it("can test all the input types", async () => {
    ...
    const later = new FormData(form.get(0) as HTMLFormElement);
  ...
  describe("checkboxes", () => {
    it("only accepts true/false for checkboxes", async () => {
      ...
      expect((input.get(0) as HTMLInputElement).value).toBe("on");
      expect((input.get(0) as HTMLInputElement).checked).toBe(false);
