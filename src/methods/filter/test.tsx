...
describe(".filter()", () => {
  ...
  it("can filter with a callback and the node", async () => {
    const hasClass = (node: Node, name: string) =>
      (node as Element).classList.contains(name);
  ...
  it("can filter with a callback and the index", async () => {
    expect(list.filter((_node, i) => i === 0).text()).toBe("A");
    expect(list.filter((_node, i) => i === 1).text()).toBe("B");
    expect(list.filter((_node, i) => i === 2).text()).toBe("Link");
  });
